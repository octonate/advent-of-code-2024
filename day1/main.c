...
int main(void) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        arr1[idx] = strToI(splitFirstTok(&lineStr, ' '));
        arr2[idx] = strToI(lineStr);
        idx++;
    }

    countSort(arr1, numLines);
    countSort(arr2, numLines);
    ...
    int simScoreSum = 0;
    for (int i = 0; i < numLines; i++) {
        simScoreSum += arr1[i] * arrCountInts(arr2, numLines, arr1[i]);
    }
